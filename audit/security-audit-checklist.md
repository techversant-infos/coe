...
**Audience:** CoE Audit Team, Security Team, Engineering Leads, all engineers
**Next Review:** December 2026

> This checklist is the organization-wide security bar for Techversant engineering. It supports the
> [CoE Audit Framework](./coe-audit-framework.md) and is structured to map one-to-one against the
> [OWASP Top 10:2025](https://owasp.org/Top10/2025/), with additional cross-cutting sections for API, AI/LLM,
> mobile, privacy, and operational security.

Referenced as the source of truth by
[Secure Engineering (Engineering Academy)](../engineering-academy/00-engineering-foundations/03-secure-engineering.md).
...
Findings are classified using [CoE Audit Framework §5](./coe-audit-framework.md) severity levels and the
SLAs in §18.2 of this document. Items marked **(B)** are **release-blocking**: a release cannot ship with an
open **(B)** item without a documented exception approved by the Security Lead
([CoE Audit Framework §8](./coe-audit-framework.md)).
...
- [ ] **(B)** Every record fetched by ID is verified as belonging to the caller before it is returned
...
- [ ] Unauthorized access to another party's record returns `404`, not `403` (does not leak existence)
...
- [ ] **(B)** File type validated by content/magic bytes, not by extension or client-supplied MIME type
...
- [ ] **(B)** Authentication endpoints rate-limited with progressive delay and lockout
...
- [ ] **(B)** Failed login attempts rate-limited, locked out, and logged (§11.1)
...
| Medium (CVSS 4.0–6.9) | 1 week | 90 days |
| Low (CVSS < 4.0) | Next sprint | Next release |
| **KEV-listed, any score** | 24 hours | 7 days |
