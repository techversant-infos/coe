# Backend Stack Translation â€” Developer Excellence Curriculum
...
- **Mini-task variant** â€” the master's mini-task, rewritten for the backend context.
...
**What changes in this stack:** A backend codebase has three layers of names â€” domain (`User`), transport (`UserDTO`), and persistence (`UserEntity`). The same user can be represented three times in three files. That's not duplication; each name is for a different audience (business logic, API contract, database).
...
**Mini-task variant:**
Find a piece of backend code you suspect is *over-abstracted* â€” a helper with only one or two callers, with a signature that's harder to read than inlining. Inline it back. Commit `refactor(clean-code-3): inline-over-abstraction`.
...
**Watch:** Search YouTube for "structured logging backend production" â€” pick a 10â€“15 min video.
...
**What changes in this stack:** Backend's subclass explosion (GoldUser, PlatinumUser, BetaUser, all copy-pasted) is the same problem as the frontend's prop explosion. The fix: a `User` with a `Tier` value object, and a service that dispatches on tier. Config-driven behavior is data â€” don't add a new subclass for every variation.
...
**What changes in this stack:** The "user" in a backend codebase is more than the end user â€” it's the *caller* of the service (another service, a script, the frontend), the *operator* (the on-call engineer), and the *auditor* (security, compliance). Change scenarios: "If we add a new tenant, what changes?" "If we move the database, what changes?" "If we add a new payment provider, what changes?" Variability points = config, DI bindings, external API clients. Stable part = domain logic.
...
**Watch:** Search YouTube for "domain modeling change scenarios" â€” pick a 10â€“20 min video.
...
**Watch:** Search YouTube for "writing testable code dependency injection" â€” pick a 10â€“15 min video.

---

## Topic 17 â€” Refactoring Techniques

**What changes in this stack:** Backend refactor decision rules â€” refactor a *service* (extract method, rename, split into two) when names are right but body is wrong; refactor a *type* (split, add a value object) when the same type is used in three places; rewrite a *module* when architecture doesn't fit. Fowler smell catalog applies: long method, large class, long parameter list, primitive obsession. "Names vs. bodies" test: if names are right, refactor; if names are wrong, rewrite.
...
**Watch:** Search YouTube for "refactoring Martin Fowler catalog" â€” pick a 10â€“15 min video.
...
**What changes in this stack:** The master's 9-point checklist applies, with backend-specific rows: **Database access** (parameterized queries, no N+1, proper indexing), **API contract** (the [REST API Best Practices](../../../../general/rest-api-best-practices.md) standards), **Auth/AuthZ** (the right user is doing the right action â€” see the [Security Audit Checklist](../../../../audit/security-audit-checklist.md)). Use your framework's static analysis tool (PHPStan, ESLint, etc.) in CI to catch the easy stuff automatically.
...
**Watch:** Search YouTube for "technical debt management" â€” pick a 10â€“15 min video.
...
| Field | Value |
|---|---|
| Document | Backend Stack Translation â€” Developer Excellence Curriculum |
| Version | 0.3 (rewrote per-topic structure to 'What changes + Mini-task + Watch'; 11 of 21 YouTube links verified by tech lead â€” topics 1, 2, 5, 6, 7, 8, 10, 14, 15, 18, 19; the rest remain search hints pending verification) |
