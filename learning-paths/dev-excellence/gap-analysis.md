...
| **Performance Awareness** (N+1, caching basics, measure-first) | ✅ | [Phase 4, Topic 14](./curriculum.md#14-performance-awareness) — full topic; pairs with the [Next.js path Phase 4](../nextjs/intermediate.md#phase-4-server-components-vs-client-components) for the caching model |
...
| **Concurrency & Threading** | ⚠️ partial | Touched implicitly in Phase 4 Topic 14 (performance) and Phase 5 Topic 16 (testability), but no dedicated topic. **Defer to a stack-specific path or a v0.2 add-on if a team needs it.** |
| **Security as a discipline** (threat modeling, OWASP, secure-by-default) | ⚠️ partial | The security *row* in the [Topic 19 checklist](./curriculum.md#19-code-review-checklist-standardized) is the entry point. Full coverage lives in the [Security Audit Checklist](../../audit/security-audit-checklist.md) and the [Auth/AuthZ topics in the stack paths](../nextjs/intermediate.md#phase-7-authentication-and-authorization). The curriculum *cites* security but does not *teach* it as a stand-alone topic. |
| **Observability & Monitoring** (structured logging, metrics, tracing) | ⚠️ partial | Touched in [Phase 1, Topic 4](./curriculum.md#4-basic-error-handling) (logging basics) and [Phase 4, Topic 14](./curriculum.md#14-performance-awareness) (measure-first). Full coverage lives in the [Node.js standards](../../nodejs/nodejs-typescript-best-practices.md) and the [Next.js path Phase 10](../nextjs/intermediate.md#phase-10-production-readiness). |
