...
**Level:** Practitioner
**Next Level:** [Advanced: Next.js](../02-nextjs/01-nextjs-learning-path.md) - complete React first
**Prerequisite for:** [Next.js Learning Path](../02-nextjs/01-nextjs-learning-path.md) finish this first
**Contributors:** Compiled by CoE Web Working Group, reviewed by Web team leads

# React Learning Path For the Web Dev Team
*From PHP/Laravel backend to confident React frontend engineer.*

This is the **foundation track**. If your team's goal is Next.js (see [../02-nextjs/01-nextjs-learning-path.md](../02-nextjs/01-nextjs-learning-path.md)), this path comes first. It covers React 19.x, JSX, components, state, effects, custom hooks, forms, API integration, testing, and production patterns all written for backend engineers who read JavaScript but haven't built a frontend professionally.

> **Architecture call (read first):** React is a **UI library**, not a full framework. You will use it as the view layer for our Laravel API. This path assumes you will **not** do full-stack React with direct DB access that's a Next.js (or Laravel Inertia) concern.
...
1. Read and write JSX confidently components, props, state, conditional rendering, lists.
...
> **Realistic scope:** ~30 hours is enough to build a working React frontend and contribute to supervised tickets. The confidence that comes from building things on the job in code review, on real tickets, pair programming is the rest of the journey.

### Team-Level Metrics of Success

Beyond individual outcomes, we measure whether the path moved the team's velocity:

- **% of frontend tickets completed without senior blocking** track per quarter, target a meaningful rise from baseline
- **PR review turnaround** on frontend PRs should drop as pattern-recognition grows
- **Defect rate** on the first 30 days after a feature merges should stay flat or fall
- **Onboarding time** for a new hire to ship their first React PR target: 2 weeks
...
The path assumes you have these technical basics. If not, spend 12 days topping up before starting:
...
| **TypeScript basics** | `interface`, `type`, generics enough to read Zod schemas and type props | [TypeScript Handbook](https://www.typescriptlang.org/docs/handbook/intro.html) |
...
> **Buffer:** treat Weeks 56 as flexible. If the team is behind, fold testing into a follow-up sprint. The point is the **6 weekly deliverables**, not hitting 6 weeks exactly.

> **Working agreement:** the deliverable at the end of each week is what gets reviewed on Friday. Pair on the harder phases (3, 4, 5). Open a PR for the deliverable even if it's small it builds the habit.

### Weekly PR Assessment Rubric

Each week's deliverable PR is reviewed on this rubric. Score 02 per row (0 = missing, 1 = partial, 2 = solid). Aim for 12/14+ before moving on. The goal isn't perfection it's catching the same gap three weeks in a row.
...
| **State placement** | State lives at the right level not too high, not too low |
...
## Phase 2 React Core: JSX, Components, and Props

**Why this phase exists:** React is the foundation everything else builds on. Get the mental model right components are functions that return UI before adding state and effects.
...
**Source of truth:** the **official React docs at [react.dev/learn](https://react.dev/learn)**. React is currently in the 19.x line always read the current docs, not a 2022 blog post.
...
- `<ProductCard />` receives a product object via props, renders name, price, image placeholder, "Add to Cart" button
- `<ProductList />` receives an array of products, maps to `<ProductCard />`
- `<App />` holds the product data, renders `<ProductList />`
...
## Phase 3 State, Events, and Conditional Rendering

**Why this phase exists:** static UI is useful, but most apps need to respond to the user. This phase introduces state the mechanism React uses to remember things between renders.
...
- **Search** filter products by name as the user types (state: `searchQuery`)
- **Category filter** filter by category (state: `selectedCategory`)
- **"Contact seller" form** controlled inputs for name, email, message (state: form fields)
- **Loading and empty states** conditional rendering based on data availability
- Use a local mock JSON file no API yet
...
## Phase 4 Effects, Refs, and the Component Lifecycle

**Why this phase exists:** most real apps need to talk to the outside world fetch data, subscribe to events, measure DOM nodes. `useEffect` is React's escape hatch for side effects. `useRef` gives you a persistent value that survives re-renders without triggering them.

**Learn:**
- `useEffect` what it does, when it runs, how the dependency array controls it
- **Effect dependency arrays:** `[]` (once on mount), `[dep]` (when dep changes), no array (every render) and the consequences of each
- **Cleanup functions** returning a function from `useEffect` to undo what you did
...
## Phase 5 Forms, Validation, and API Integration
...
- **React Hook Form** register inputs, handle submission, show errors, reset with minimal re-renders
- **Zod** for schema validation (matches our Node.js standard) infer TypeScript types from schemas
...
- *(Forward-looking)* React 19's `useActionState` and **Server Actions** are the natural extension of the React Hook Form + Zod pattern when you move to Next.js covered in the [Next.js Learning Path](../02-nextjs/01-nextjs-learning-path.md#phase-6-forms-validation-and-mutations). For pure React (Vite, CRA, etc.), stick with React Hook Form.
...
- "Add Customer" button POST to a Laravel endpoint (mock or real)
...
## Phase 6 Composition Patterns and Custom Hooks
...
- **Component composition deep dive:**
 ...
 - **Compound components:** `<Select><SelectTrigger /><SelectContent /></Select>` components that share implicit state
...
- **Custom hooks:** extracting logic into a reusable function prefixed with `use`
 - `useFetch(url)` fetch + loading + error + data
 - `useLocalStorage(key, initial)` persist state to localStorage
 - `useDebounce(value, delay)` delay a value change
 - `useAuth()` wrap the Laravel auth check
- **Context API:** `createContext`, `useContext`, `Provider` for data that's needed deeply without prop drilling
...
## Phase 7 Testing React Components

**Why this phase exists:** we have a Node.js testing standard ([Node.js TypeScript Best Practices](../../../../../nodejs/nodejs-typescript-best-practices.md)) the React path extends it.

**Learn:**
- **Vitest** unit test runner (or Jest if a project predates it)
- **React Testing Library (RTL)** test components the way users interact with them
 - Query by role, label, text **not** by class name or internal state
 - `render`, `fireEvent`, `screen`, `within`
- **MSW (Mock Service Worker)** intercept API calls at the network level, return fake data
 - Same handlers run in Vitest (Node) and the browser one definition, both environments
- **Testing patterns:**
...
**Mini task:**
- RTL test for `<ProductList />` renders products, filters when search input changes, shows loading state
...
## Phase 8 Production Patterns and Project Structure
...
- **Folder structure for a React project:**
 ...
 features/ vertical slices: customers/, products/
 customers/
 components/
 hooks/
 ...
 lib/ framework glue (fetch client, env, utils)
...
**Reading order:** official sources first they are the most current. Treat third-party content as supplements.
...
|---|---|---|---|
| **Primary React** | [react.dev/learn](https://react.dev/learn) official React docs (Quick Start, Thinking in React, Managing State) | Official, current, project-based, maintained by the React team | Free |
| **Supplement interactive** | [Scrimba: Learn React](https://scrimba.com/learn/learnreact) video + embedded editor, pause-and-edit | Best for visual / kinesthetic learners; type along without setting up a project | Free (Scrimba account) |
| **Supplement full video build** | *Modern React Full Course 2026* ByteGrad or freeCodeCamp on YouTube (search for the latest) | Single sitting, full build from zero to deploy; pick a 2025/2026 release | Free |
| Deep dive | *The Road to React* Robin Wieruch (free online) | Written for developers, concise, covers hooks well | Free |
| TypeScript + React | *Total TypeScript* Matt Pocock | Best-in-class for the TypeScript + React typing patterns we use | Paid |
...
Curated, low-noise, high-signal. Subscribe to 23 more than that and you'll stop reading them.

**YouTube channels**
- [React](https://www.youtube.com/@react) official React channel
- [Lee Robinson](https://www.youtube.com/@leerob) Vercel VP, practical React + Next.js patterns
...
**Podcasts**
- *Syntax.fm* weekly, broad web dev with regular React episodes
...
**Blogs to follow**
- [React Blog](https://react.dev/blog) official releases and deep dives
...
**Curated tutorials**
- [Scrimba: Learn React](https://scrimba.com/learn/learnreact) interactive, pause-and-edit
- [Patterns.dev](https://www.patterns.dev/) modern web app patterns (free book)
- [The Road to React](https://www.roadtoreact.com/) Robin Wieruch, written for developers

**Recipes & cookbooks**
- [React Examples repo](https://github.com/facebook/react/tree/main/docs/src/components/examples) official examples
- [useHooks](https://usehooks.com/) collection of curated React hooks with explanations
...
|---|---|---|---|
| **Meta Front-End Developer Professional Certificate** | Coursera / Meta | Covers React fundamentals, JS, CSS useful if Phase 2 feels shaky | Paid |
...
**This is v0.1 Draft for Pilot Batch.** Run it with 35 developers first. Collect feedback after Week 3 and revise before rolling out to the full team.
...
5. **Use AI carefully.** Per [CoE AI guidelines](../../../../../general/ai-era-coding-guidelines.md), AI can *explain concepts*, *scaffold tests* (Phase 7 especially), and *review* code but **auth and DB migration code is Red Zone, manual only.** Tag AI-assisted commits `[ai-assisted: <tool>]`.
...
7. **Then move to Next.js.** This path feeds directly into [../02-nextjs/01-nextjs-learning-path.md](../02-nextjs/01-nextjs-learning-path.md). Once you've completed this path, the Next.js path will feel like a natural extension not a completely different world.
...
| Supersedes | v0.2 |
| Related | [Next.js Learning Path](../02-nextjs/01-nextjs-learning-path.md), [Node.js TypeScript Best Practices](../../../../../nodejs/nodejs-typescript-best-practices.md), [REST API Best Practices](../../../../../general/rest-api-best-practices.md), [AI Era Coding Guidelines](../../../../../general/ai-era-coding-guidelines.md) |
