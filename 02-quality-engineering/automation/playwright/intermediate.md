# Automation Engineer Learning Path (Manual Tester â†’ Automation Engineer)

> **Objective:** Transform manual testers into automation engineers who can own end-to-end and API test automation â€” using Playwright as the CoE standard.
> **Audience:** Manual testers who want to automate + existing automation engineers who want to mature. Two tracks in one path.
> **Read alongside:** [README.md](./README.md) (prerequisites, entry points), [REST API Best Practices](../../../general/rest-api-best-practices.md) (Phase 4), [Dev Excellence Curriculum](../../../00-engineering-foundations/dev-excellence/curriculum.md) (discipline).
> **Status:** Draft for pilot batch review â€” v0.1.
...
- Write automated tests that verify user workflows end-to-end â€” not just unit checks
- Apply test design patterns (page objects, parameterized tests, fixtures) that survive code changes
- Automate API testing alongside E2E â€” the full coverage picture
- Integrate tests into CI/CD pipelines â€” automated, not manual
- Report test results effectively â€” clear pass/fail, flaky detection, accessibility metrics
- Mentor manual testers on automation â€” the career path they wish they'd had

---

## ðŸ“š Table of Contents

### ðŸ”µ Manual Tester Track (Phases 1-3)
...
### ðŸŸ¡ Automation Engineer Track (Phases 4-6)
...
## ðŸ”µ Phase 1 â€” Automation Mindset (Beginner)
...
- **Manual vs. Automated Testing â€” The Paradigm Shift:** Manual testing is exploratory; automated testing is deterministic. A manual tester asks "what happens if I do this?" An automation engineer asks "how do I verify this happens every time I run the test?"
- **WhatAutomates Well vs. What Doesn't:** High-value automations â€” regression suites, smoke tests, critical user journeys. Low-value automations â€” one-off exploratory tests, UI that changes weekly. The ROI test: "will I run this 10+ times?"
- **The Automation Pyramid:** Unit tests (fast, brittle) â†’ Integration tests (medium speed, medium value) â†’ E2E tests (slow, highest confidence). Automate the pyramid; don't skip levels.
...
- **The Debug Flow:** (1) Read the error message â€” it tells you exactly what failed. (2) Check the timeline â€” Playwright records video/network on failure. (3) Verify the locator â€” is the element still there? (4) Isolate the test â€” run just the failing test to reproduce. (5) Fix and verify.
...
## ðŸ”µ Phase 2 â€” Playwright Fundamentals (Intermediate)
...
- **Locator Priority:** (1) Test IDs (`data-testid`) â€” most stable. (2) Role + accessible name (`getByRole('button', { name: 'Submit' })`) â€” semantic. (3) Text (`getByText('Submit')`) â€” fragile. (4) CSS/XPath â€” last resort.
...
- **Playwright Actions:** `click()`, `fill()`, `type()`, `selectOption()`, `check()`, `uncheck()`, `hover()`. These auto-wait â€” Playwright waits for the element to be actionable before acting.
...
- **Soft Assertions vs. Hard Assertions:** Hard â€” the test stops on the first failure. Soft (`expect.soft()`) â€” continues even if it fails, then reports all failures at the end. Use soft for comprehensive reporting.
- **Custom Matchers:** `expect.extend()` to add project-specific matchers. The CoE standardizes on a few custom matchers â€” learn the standard ones first.
...
## ðŸ”µ Phase 3 â€” Test Design & Maintenance (Intermediate)
...
- **Test File Structure:** One test file per feature or user journey. Tests should be independent â€” no ordering dependencies. Use `test.describe()` to group related tests.
...
- **Environment-Specific Configs:** Dev, staging, production â€” different URLs, different credentials. Use `playwright.config.ts` projects and environment variables.
...
- **The 5 Maintenance Rules:** (1) Prefer semantic locators (roles, labels). (2) Test behavior, not implementation. (3) Use `waitFor()` for dynamic content (not `sleep()`). (4) Independent tests â€” no shared state. (5) Name tests like documentation.
- **Flaky Test Management:** Retry once locally; if it fails twice, it's not flaky â€” it's broken. Track flaky tests separately (`--retries`).
...
- **Playwright Trace Viewer:** `npx playwright show-report` â€” see every action, every network request, every console log. The best debugging tool in testing.
...
## ðŸŸ¡ Phase 4 â€” API Automation (Advanced)
...
- **Response Verification:** `expect(response).toBeOK()`, `expect(response).toHaveBody()`, `expect(response).toHaveHeader()`. API assertions are simpler than UI â€” status codes, headers, body.
...
- **Session Management in Tests:** Login once, reuse the session. Playwright's `storageState` â€” authenticate in one test, save state, use it in others. No re-login in every test.
...
- **Test Data Lifecycle:** Setup â†’ Test â†’ Teardown. Create data before the test, clean it up after. Playwright `test.afterEach()` for teardown.
...
- [ ] I understand the setup â†’ test â†’ teardown lifecycle
...
- **Error Case Testing:** Negative assertions. `toHaveStatus(401)`, `toHaveErrorBody()`. Test the error paths â€” they're as important as success paths.
...
## ðŸŸ¡ Phase 5 â€” CI/CD Integration (Advanced)
...
- **Configuring Workers:** `workers: process.env.CI ? 4 : 1` â€” parallel in CI, serial locally for debugging. The CoE standard configuration.
...
## ðŸŸ¡ Phase 6 â€” Quality & Maturity (Leadership Track)
...
- **Performance Testing with Playwright:** `playwright test --project=performance` â€” measure page load, time to interactive. Integrate with CI thresholds.
...
| Version | 0.1 (draft â€” 6 phases, 22 topics) |
| Owner | CoE QA Working Group |
| Review Cycle | Quarterly |
| Status | Draft for pilot batch review |
| Related | [README.md](./README.md), [roadmap-gap-analysis.md](./roadmap-gap-analysis.md), [REST API Best Practices](../../../general/rest-api-best-practices.md), [Dev Excellence Curriculum](../../../00-engineering-foundations/dev-excellence/curriculum.md) |
