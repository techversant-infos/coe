...
**ChatGPT note:** Use ChatGPT for general Q&A and exploration only. It has no access to your codebase, project history, or team standards — every session starts from zero. Do not use it for project-specific code decisions.

**Model selection rule:** Claude Opus is reserved for architecture decisions and complex reasoning only. Sonnet for daily dev work. Haiku for repetitive tasks (test generation, doc summaries, fast completions).

**Design tools:** v0.dev, Figma AI, and design-to-code workflows are not covered by this standard. A separate AI-for-design playbook applies to UI/UX roles.
...
Use this framework before every AI-assisted task. Levels are colour-coded to match the presentation's delegation scale.

| Level | GPS Label | When | Examples |
|-------|-----------|------|---------|
| **Full Delegate** | 🟢 GREEN — Follow GPS | Output is mechanical, verifiable, low-risk | Boilerplate, CRUD scaffolding, test stubs, PR descriptions |
| **Collaborate** | 🟡 YELLOW — Confirm the route | Needs domain judgment to validate | Business logic, integrations, query optimisation, refactoring |
| **Human-led** | 🟠 AMBER — You navigate | Requires your architectural context | System design, API contracts, data models, critical flows |
| **Never Delegate** | 🔴 RED — GPS off | Sensitive, irreversible, or security-critical | Auth, encryption, PII, production incidents, architecture decisions |
...
5. No AI agent has write access to production systems, and no AI agent pushes directly to main or production branches
...
Track these four metrics every sprint. They are the primary indicators that AI is improving your workflow — not just changing it.
...
**Supplementary (track quarterly):**

| Metric | How to measure | Target direction |
|--------|---------------|-----------------|
| Test coverage delta | Coverage % before vs. after sprint | ↑ |
| Story points per developer | Velocity from sprint board | Track trend |
...
## 13. First Sprint Actions

Run these within the first sprint of adopting this standard. These are the minimum viable actions — not aspirational goals.

| Who | Action | When |
|-----|--------|------|
| All developers | Set up CLAUDE.md on top 2 active projects | This sprint |
| All developers | Log AI task types in sprint notes | This sprint |
| Tech leads | Add `ai-assisted` label to PR template | This week |
| Architects | Define and document AI off-limits zones per project | This week |

**Why this order:** CLAUDE.md and PR tagging have the highest leverage-to-effort ratio. Do those two first. Everything else builds on them.

**After the first sprint:** Review your four primary metrics. Baseline them. You cannot improve a route you do not measure.
...
| 1.1 | 2026-04-21 | Aligned with GPS presentation — delegation labels, tool selection, governance rule consolidation, metrics split (primary/supplementary), first sprint actions added |
